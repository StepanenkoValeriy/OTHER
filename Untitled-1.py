...
def input_error(func):  #dec
    def inner(*args, **kwargs):
        try:
            return func(*args, **kwargs)
        except ValueError as e:
            if str(e)=="wrong date":
                return f"Date format should be DD.MM.YYYY"
            return "You haven't entered a value. Nothing to add/change."
        except KeyError:
            return "Missed contact. Please add contact "
        except IndexError:
            return "Missed name of contact" 
...
def save_data(book:AddressBook, filename="addressbook.pkl"):#Saving if file exist, or creating new
    with open(filename, "wb") as f:
        pickle.dump(book, f)

def load_data(filename="addressbook.pkl"):#Loading from file
    try:
...
def main():
    book = load_data() #loading data from addressbook.pkl
    print("Welcome to the assistant bot! \n close/exit - quit BOT \n add/change name phone - adding or changing contact\n add-birthday name date - adding birthday to contact \n birthdays - upcomming birthdays \n all - show all contacts")
    while True:
        user_input = input("Enter a command: ")
        command, *args = parse_input(user_input)

        if command in ["close", "exit"]:
            save_data(book)#Saving data
            print("Good bye!")
            break
